...
export default function AuthLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ Save token + user info (for cart and Redux)
...
const handleGoogleSuccess = async (credentialResponse) => {
  try {
    ...
    // ✅ Ensure _id exists for Redux/cart logic
    const userWithId = {
      ...data.user,
      _id: data.user.id || data.user.id || data.user.sub,
    };

    // ✅ Save token + user info
    if (data.token) localStorage.setItem("token", data.token);
    localStorage.setItem("user", JSON.stringify(userWithId));
    if (userWithId.role) localStorage.setItem("userRole", userWithId.role.toLowerCase());

    // ✅ Sync Redux
    dispatch(setUser({ user: userWithId, token: data.token }));

    // ✅ Verify auth in Redux
    dispatch(checkAuth());
